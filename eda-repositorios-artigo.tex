...
Quando se fala de distribuição de computação e replicabilidade, surge imediatamente a necessidade de replicar também todo o ambiente de execução no qual essa computação é executada. Sem uma solução de virtualização, isso implica a instalação de um sistema operativo e de todas as dependências relevantes para a execução do código relevante. Este é um processo manual e dispendioso que torna claramente necessário assegurar a portabilidade do ambiente de execução, tornando-o instalável automaticamente em qualquer nó com o mínimo de dependências. 
...
\subsection{Blockchain e dados abertos}

Num futuro mais descentralizado será necessário conceber soluções para assegurar a autenticidade da informação colocada em cada um dos repositórios de uma eventual federação. É claro o potencial de aplicação de tecnologias como a \emph{blockchain} aos diversos aspetos da gestão de dados abertos:
...
\begin{itemize}
	...
	\item \textbf{Comprovação de resultados}
\end{itemize}
